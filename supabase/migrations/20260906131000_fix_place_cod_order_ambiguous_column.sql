-- After fixing the search_path (previous migration), a second bug surfaced:
-- the idempotency-check SELECT read unqualified order_id/order_code/order_total
-- columns from private.cod_checkout_idempotency, which collided with the
-- function's own RETURNS TABLE(order_id, order_code, order_total) OUT params:
--   ERROR: column reference "order_id" is ambiguous
-- Fix: alias the table and qualify the columns.
CREATE OR REPLACE FUNCTION public.place_cod_order(_customer_name text, _mobile text, _village text, _address text, _pincode text, _items jsonb, _remarks text DEFAULT NULL::text, _idempotency_key text DEFAULT NULL::text)
...
BEGIN
  ...
  -- Count every invocation, including malformed/failed attempts, so validation cannot be used to bypass throttling.
  ...
  END IF;
  INSERT INTO private.cod_checkout_rate_limits(mobile,client_ip,device_key) VALUES(CASE WHEN _normalized_mobile~'^[0-9]{10}$' THEN _normalized_mobile ELSE '__invalid__' END,_client_ip,_device_key);
  ...
  SELECT t.payload_hash,t.mobile,t.order_id,t.order_code,t.order_total INTO _existing_payload_hash,_existing_mobile,_existing_order_id,_existing_order_code,_existing_order_total FROM private.cod_checkout_idempotency t WHERE t.idempotency_hash=_idempotency_hash FOR UPDATE;
  IF FOUND THEN
