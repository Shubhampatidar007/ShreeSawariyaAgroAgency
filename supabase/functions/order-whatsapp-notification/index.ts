...
Deno.serve(async (req) => {
  ...
  const to = normalizePhone(ADMIN_WHATSAPP_NUMBER);
  const endpoint = `https://graph.facebook.com/${WHATSAPP_API_VERSION}/${phoneNumberId}/messages`;
  ...
  try {
    ...
    if (!response.ok || !messageId) {
      return json(
        {
          ok: false,
          error: body?.error?.message || `Meta returned HTTP ${response.status}.`,
        },
        502,
      );
    }
    ...
    await admin.from("reminder_logs").insert({
      reminder_title: "WhatsApp online order notification",
      recipient: order.customer_name || order.mobile || "Online customer",
      channel: "whatsapp",
      sent_at: new Date().toISOString(),
      delivery: "sent",
    ...
    return json({
      ...
      orderId: order.id,
      orderCode: order.code,
      note: "Order notification accepted by Meta Cloud API.",
    });
  } catch (error) {
    return json(
      {
