...
const messagePresets: Record<WhatsAppMessageKind, { label: string; description: string; message: string }> = {
  ...
  custom: {
    ...
    description: "Edit the message before sending.",
...
export const Route = createFileRoute("/admin/reminders")({
  head: () => ({
    meta: [
      ...
      { name: "description", content: "Preview customer purchase records and filter customers for WhatsApp reminders." },
...
function RemindersPage() {
  ...
  const filteredCustomers = useMemo(() => {
    ...
    const minPurchase = filters.minPurchase === "" ? Number.NEGATIVE_INFINITY : Number(filters.minPurchase);
    const maxPurchase = filters.maxPurchase === "" ? Number.POSITIVE_INFINITY : Number(filters.maxPurchase);

    return customers.filter((customer) => {
      const queryMatch = !q || `${customer.name} ${customer.mobile} ${customer.village}`.toLowerCase().includes(q);
      ...
      const purchaseMatch = customer.totalPurchases >= minPurchase && customer.totalPurchases <= maxPurchase;
      ...
      return queryMatch && villageMatch && dueMatch && purchaseMatch && dueOnlyMatch && whatsappMatch && fromMatch && toMatch;
    });
  }, [customers, filters]);
  ...
  const previewText = (customer: Customer) => {
    ...
    return message
      ...
      .replaceAll("{lastPurchase}", latest ? formatDate(latest.date) : customer.lastPurchase ? formatDate(customer.lastPurchase) : "—");
  };
  ...
  return (
    <div className="relative space-y-6">
      <ModulePageHeader
        ...
        actions={<Badge variant="outline" className="h-9 rounded-full px-3"><span className="mr-2 size-2 rounded-full bg-emerald-500" />Live WhatsApp</Badge>}
      />

      <div className="grid gap-4 md:grid-cols-3">
        <Metric label="Customers matched" value={String(filteredCustomers.length)} helper={`${selectedCustomers.length} selected`} />
        <Metric label="Selected due" value={formatCurrency(selectedDue)} helper="Outstanding across selected customers" />
      ...
      {result ? (
        <div className="flex items-start gap-3 rounded-2xl border border-border bg-card p-4 shadow-soft">
          {result.ok ? <CheckCircle2 className="mt-0.5 size-5 text-emerald-600" /> : <X className="mt-0.5 size-5 text-destructive" />}
        ...
        <TabsContent value="send" className="space-y-5">
          ...
          <Card className="shadow-soft">
            ...
            <CardContent className="space-y-4">
              <div className="grid gap-3 md:grid-cols-2 xl:grid-cols-4">
                <div className="relative xl:col-span-2"><Search className="absolute left-3 top-3 size-4 text-muted-foreground" /><Input className="pl-9" placeholder="Name, mobile or village" value={filters.query} onChange={(e) => setFilters({ ...filters, query: e.target.value })} /></div>
                <select className="h-10 rounded-md border border-input bg-background px-3 text-sm" value={filters.village} onChange={(e) => setFilters({ ...filters, village: e.target.value })}><option value="all">All villages</option>{villages.map((village) => <option key={village} value={village}>{village}</option>)}</select>
                <Button variant={filters.dueOnly ? "default" : "outline"} className="rounded-full" onClick={() => setFilters({ ...filters, dueOnly: !filters.dueOnly })}>Due only: {filters.dueOnly ? "On" : "Off"}</Button>
                <Input type="number" min="0" placeholder="Minimum due" value={filters.minDue} onChange={(e) => setFilters({ ...filters, minDue: e.target.value })} />
                <Input type="number" min="0" placeholder="Maximum due" value={filters.maxDue} onChange={(e) => setFilters({ ...filters, maxDue: e.target.value })} />
                <Input type="number" min="0" placeholder="Minimum purchase" value={filters.minPurchase} onChange={(e) => setFilters({ ...filters, minPurchase: e.target.value })} />
                <Input type="number" min="0" placeholder="Maximum purchase" value={filters.maxPurchase} onChange={(e) => setFilters({ ...filters, maxPurchase: e.target.value })} />
                <Input type="date" value={filters.purchaseFrom} onChange={(e) => setFilters({ ...filters, purchaseFrom: e.target.value })} />
                <Input type="date" value={filters.purchaseTo} onChange={(e) => setFilters({ ...filters, purchaseTo: e.target.value })} />
                <Button variant={filters.whatsappOnly ? "default" : "outline"} className="rounded-full" onClick={() => setFilters({ ...filters, whatsappOnly: !filters.whatsappOnly })}><Smartphone className="size-4" /> WhatsApp only: {filters.whatsappOnly ? "On" : "Off"}</Button>
                <Button variant="outline" className="rounded-full" onClick={() => setFilters(initialFilters)}>Reset filters</Button>
              </div>
            </CardContent>
          </Card>

          <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
            <div className="flex gap-2"><Button variant="outline" className="rounded-full" onClick={() => setSelectedIds(new Set(filteredCustomers.map((customer) => customer.id)))}>Select all</Button><Button variant="ghost" className="rounded-full" onClick={() => setSelectedIds(new Set())}>Clear</Button></div>
            <p className="text-sm text-muted-foreground">{filteredCustomers.length} customers · {selectedCustomers.length} selected</p>
          </div>
          ...
          <div className="grid gap-3 lg:grid-cols-2">
            {filteredCustomers.map((customer) => {
              ...
              return (
                <Card key={customer.id} className={selected ? "border-primary shadow-soft" : "shadow-soft"}>
                  <CardContent className="flex items-center gap-3 p-4">
                    <Button size="sm" variant={selected ? "default" : "outline"} className="size-9 rounded-full p-0" onClick={() => setSelectedIds((current) => { const next = new Set(current); if (next.has(customer.id)) next.delete(customer.id); else next.add(customer.id); return next; })}>{selected ? "✓" : ""}</Button>
                    <div className="min-w-0 flex-1"><p className="truncate font-semibold">{customer.name}</p><p className="truncate text-xs text-muted-foreground">{customer.mobile} · {customer.village || "No village"}</p><p className="mt-1 text-xs">Purchase {formatCurrency(customer.totalPurchases)} · Due <span className="font-semibold">{formatCurrency(customer.currentDue)}</span></p></div>
                    <Button size="sm" variant="outline" className="rounded-full" onClick={() => setPreviewCustomer(customer)}><Eye className="size-4" /> Preview</Button>
                  </CardContent>
                </Card>
              );
            })}
...
function CustomerPreviewDialog({ customer, ledger, message, onMessageChange, onClose, onSend, sending }: { customer: Customer | null; ledger: CustomerLedgerEntry[]; message: string; onMessageChange: (value: string) => void; onClose: () => void; onSend: () => void; sending: boolean }) {
  const [items, setItems] = useState<CustomerSaleItem[]>([]);
  const [loadingItems, setLoadingItems] = useState(false);

  const records = useMemo(() => customer ? ledger.filter((entry) => entry.customerId === customer.id).sort((a, b) => b.date.localeCompare(a.date)) : [], [customer, ledger]);
  ...
  useEffect(() => {
    ...
    void shopStore.fetchTransactionItems(latestPurchase.id).then((next) => { if (active) setItems(next); }).catch(() => { if (active) setItems([]); }).finally(() => { if (active) setLoadingItems(false); });
    return () => { active = false; };
  }, [latestPurchase?.id]);
  ...
  const preview = message
    ...
    .replaceAll("{totalPurchase}", formatCurrency(customer.totalPurchases))
    .replaceAll("{totalPaid}", formatCurrency(customer.totalPaid))
    .replaceAll("{lastPurchase}", latestPurchase ? formatDate(latestPurchase.date) : customer.lastPurchase ? formatDate(customer.lastPurchase) : "—");
