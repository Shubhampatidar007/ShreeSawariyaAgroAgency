...
function getConfig(): WhatsAppConfig {
  ...
  return { apiVersion: process.env.WHATSAPP_API_VERSION?.trim() || DEFAULT_API_VERSION, phoneNumberId, accessToken, recipientPhone };
}
...
function interpolateTemplate(message: string, recipient: { name: string; due: number }): string {
  return message.replaceAll("{name}", recipient.name).replaceAll("{due}", `₹${recipient.due.toLocaleString("en-IN")}`);
}

export async function sendWhatsAppText(recipient: { name: string; due: number; message: string }) {
  ...
  const response = await fetch(endpoint, {
    ...
    body: JSON.stringify({ messaging_product: "whatsapp", recipient_type: "individual", to, type: "text", text: { preview_url: false, body: bodyText } }),
  });

  const body = (await response.json().catch(() => null)) as { messages?: Array<{ id?: string }>; error?: { message?: string } } | null;
  if (!response.ok) throw new Error(body?.error?.message || `Meta WhatsApp API returned HTTP ${response.status}.`);
