...
const toTestimonial = (row: TestimonialRow): Testimonial => ({
  id: row.id,
  farmerName: row.farmer_name?.trim() || "Farmer",
  farmName: row.farm_name?.trim() || undefined,
  ...
  loadPromise = (async () => {
    ...
    try {
      ...
      const [productsResult, testimonialsResult, cmsResult, advertisementsResult] = await Promise.all([
        supabase
          .from("products")
          .select("id, inventory_id, title, category, selling_price, discount_price, stock, description, tags, images, emoji, visibility, featured, status, published_on")
      ...
      setState({
        ...
        testimonials: testimonialsResult.error
          ? []
          : (testimonialsResult.data ?? []).map((row) => toTestimonial(row as TestimonialRow)),
        cmsSections: cmsResult.error
          ? []
          : (cmsResult.data ?? []).map((row) => toCmsSection(row as CmsRow)),
        advertisements: advertisementsResult.error
          ? []
          : (advertisementsResult.data ?? []).map((row) => toAdvertisement(row as AdvertisementRow)),
        loading: false,
        error: null,
      });
      loadedAt = Date.now();
