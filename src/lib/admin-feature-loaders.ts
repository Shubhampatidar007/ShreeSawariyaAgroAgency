...
export async function loadCustomersFeature() { const { data, error } = await supabase.from("customers").select("*").order("name"); if (error) throw error; return (data ?? []).map(toCustomer); }
export async function loadSuppliersFeature() { const { data, error } = await supabase.from("suppliers").select("*").order("name"); if (error) throw error; return (data ?? []).map(toSupplier); }
export async function loadProductsFeature() { const { data, error } = await supabase.from("products").select("*").order("published_on", { ascending: false }); if (error) throw error; return (data ?? []).map(toProduct); }
export async function loadInventoryFeature() {
  ...
  if (inventory.error) throw inventory.error; if (reminders.error) throw reminders.error; if (products.error) throw products.error;
  return { inventory: (inventory.data ?? []).map(toInventory), reminders: (reminders.data ?? []).map(toReminder), products: (products.data ?? []).map(toProduct) };
}
export async function loadSalesFeature() {
  ...
  if (orders.error) throw orders.error; if (customerLedger.error) throw customerLedger.error;
  return { orders: (orders.data ?? []).map(toOrder), customerLedger: (customerLedger.data ?? []).map(toCustomerLedger) };
}
export async function loadPaymentsFeature() { const { data, error } = await supabase.from("payments").select("*").order("entry_date", { ascending: false }); if (error) throw error; return (data ?? []).map(toPayment); }
export async function loadRemindersFeature() {
  ...
  if (reminders.error) throw reminders.error; if (logs.error) throw logs.error;
  return { reminders: (reminders.data ?? []).map(toReminder), reminderLogs: logs.data ?? [] };
}
