...
export async function loadPublicShopData() {
  try {
    ...
    const firstError = [products, cmsSections].find((result) => result.error);
...
export async function loadAdminShopData() {
...
export function initAdminShopData() {
  ...
  if (!adminLoadPromise) {
    adminLoadPromise = loadAdminShopData().catch((error) => {
