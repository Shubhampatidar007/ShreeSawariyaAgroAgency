...
export function KhataSaleDialog({ customer, trigger, onCreated }: Props) {
  ...
  // customer selection (only relevant when no `customer` prop is given)
  const [customerMode, setCustomerMode] = useState<"select" | "new">("select");
  const [selectedCustomerId, setSelectedCustomerId] = useState<string | null>(null);
  const [customerQuery, setCustomerQuery] = useState("");
  const [newCustomer, setNewCustomer] = useState({ name: "", mobile: "", village: "", address: "" });
  ...
  const total = useMemo(() => items.reduce((s, i) => s + i.quantity * i.rate, 0), [items]);
  ...
  const filteredProducts = useMemo(() => {
    ...
    return pool.filter((p) => p.title.toLowerCase().includes(q)).slice(0, 8);
    ...
    setItems((prev) => {
      ...
      return [
        ...prev,
        {
          ...
          quantity: 1,
          maxStock: inv ? inv.quantity : product.stock,
        },
      ];
    });
  };
  ...
  const addCustomItem = () => {
    ...
    setItems((prev) => [
      ...prev,
      { key: crypto.randomUUID(), product: customName.trim(), unit: customUnit || "unit", rate, quantity: qty },
    ]);
    setCustomName("");
    setCustomRate("");
    setCustomQty("1");
      ...
      <DialogContent className="max-h-[90vh] max-w-2xl overflow-y-auto">
        <DialogHeader>
          <DialogTitle className="flex items-center gap-2">
            <ShoppingCart className="size-5" /> New khata sale
          </DialogTitle>
          <DialogDescription>
            {customer ? `Recording a sale for ${customer.name}` : "Select or create a customer, add products, then save."}
          </DialogDescription>
        </DialogHeader>
        ...
        <div className="space-y-6">
              ...
              {customerMode === "select" ? (
                ...
                <div className="grid gap-2 sm:grid-cols-2">
                  ...
                  <Input
                    ...
                    onChange={(e) => setNewCustomer((v) => ({ ...v, village: e.target.value }))}
                  />
                  <Input
                    placeholder="Address"
          ...
          <div className="space-y-3">
            ...
            <div className="flex flex-wrap gap-2">
              {filteredProducts.map((p) => (
                ...
                >
                  {p.emoji} {p.title} · {formatCurrency(p.sellingPrice)}
                </Button>
              ))}
            ...
            <div className="grid gap-2 rounded-lg border p-3 sm:grid-cols-[1fr_auto_auto_auto]">
              ...
              />
              <Button type="button" size="sm" variant="secondary" className="sm:col-span-4" onClick={addCustomItem}>
          ...
          <div className="flex items-center justify-between rounded-lg bg-muted p-3 text-sm">
            ...
            <span>Paid: <strong className="text-success">{formatCurrency(paidNum)}</strong></span>
            <span>Due: <strong className={due > 0 ? "text-warning" : "text-success"}>{formatCurrency(due)}</strong></span>
          </div>
        </div>
        ...
        <DialogFooter>
          ...
          <Button type="button" className="rounded-full" onClick={handleSubmit} disabled={submitting}>
            {submitting && <Loader2 className="size-4 animate-spin" />} Save sale
          </Button>
        </DialogFooter>
      </DialogContent>
    </Dialog>
