...
export function AboutGithubProfile() {
  ...
  return (
    <section
      id="github"
      className="relative overflow-hidden border-t border-white/10 bg-[#050805] py-28 sm:py-40"
    >
      <div
      ...
      <div className="about-scroll-content relative z-[1] mx-auto w-[min(100%-1.25rem,1200px)]">
        <p
          className="about-kicker text-xs uppercase tracking-[0.3em] text-emerald-300/70"
        ...
        <AnimatedText
          as="h2"
          text="Code lives on GitHub"
          className="mt-4 text-4xl font-semibold leading-[1.05] text-white sm:text-5xl lg:text-6xl"
          splitBy="lines"
        />
        <p
  ...
  return (
    <div className="flex flex-col items-start gap-5">
      <img
        ...
        className="h-20 w-20 rounded-full border border-white/15 sm:h-24 sm:w-24"
        loading="lazy"
        width={96}
        height={96}
      />
      ...
      <div className="flex flex-wrap gap-2 text-xs text-white/50">
        ...
        {user.blog && (
          <a
            ...
            target="_blank"
            rel="noreferrer"
            className="inline-flex items-center gap-1.5 rounded-full border border-white/10 px-3 py-1 transition-colors hover:border-emerald-400/40 hover:text-emerald-300"
          >
      ...
      <dl className="grid w-full grid-cols-3 gap-3 border-t border-white/10 pt-5 text-center">
        <div>
          <dt className="text-[11px] uppercase tracking-wide text-white/40">Repos</dt>
          <dd className="mt-1 text-lg font-semibold text-white">{user.public_repos}</dd>
        </div>
        <div>
          <dt className="text-[11px] uppercase tracking-wide text-white/40">Followers</dt>
          <dd className="mt-1 text-lg font-semibold text-white">{user.followers}</dd>
        </div>
        <div>
          <dt className="text-[11px] uppercase tracking-wide text-white/40">Following</dt>
          <dd className="mt-1 text-lg font-semibold text-white">{user.following}</dd>
        </div>
      </dl>
      ...
      <a
        ...
        target="_blank"
        rel="noreferrer"
        className="mt-2 inline-flex items-center gap-2 rounded-full bg-emerald-400/10 px-4 py-2 text-sm font-medium text-emerald-300 transition-colors hover:bg-emerald-400/20"
      >
...
function RepoCard({ repo }: { repo: GithubRepo }) {
  return (
    <a
      ...
      target="_blank"
      rel="noreferrer"
      className="group flex flex-col justify-between rounded-2xl border border-white/10 bg-white/[0.02] p-5 transition-colors hover:border-emerald-400/30 hover:bg-white/[0.04]"
    >
      <div>
        <h4 className="truncate text-sm font-semibold text-white group-hover:text-emerald-300">
          {repo.name}
        </h4>
      ...
      <div className="mt-4 flex items-center gap-4 text-xs text-white/45">
        {repo.language && (
          <span className="inline-flex items-center gap-1.5">
            <span className="h-2 w-2 rounded-full bg-emerald-400" /> {repo.language}
          </span>
        )}
...
function ErrorState({ label }: { label: string }) {
  return (
    <p className="rounded-xl border border-red-400/20 bg-red-400/5 p-4 text-sm text-red-200/80">
      Couldn't load {label} right now — GitHub's public API may be rate-limited. Try again shortly.
    </p>
  );
}